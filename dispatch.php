...
// Start the connection to database pessdb
require_once 'db.php';
...
// create variable result to contain the result-set from SQL query
...
while ($row = $result -> fetch_assoc()) {
	...
	// create array var car to contain the column of a row
...
if ($btnDispatchClicked == true) {
	...
	// create var ncidentStatus with initial value 0
	$incidentStatus = 0;
	...
	$sql = "INSERT INTO incident(caller_name, phone_number, incident_type_id, incident_location, incident_desc, incident_status_id) VALUES ('" . $callerName . "', '" . $contactNo . "', '" . $typeofIncident . "', '" . $locationofIncident . "', '" . $descriptionofIncident . "', '" . $incidentStatus . "')";
		...
		// create var updateSuccess and var insertDisapatchSuccess with initial value false
		$updateSuccess =  false;
		$insertDisapatchSuccess = false;
		...
		for ($i=0; $i<$numofPatrolcarDispatched; $i++) {
			...
			$insertDisapatchSuccess = $conn -> query($sql);
			// display error msg if unable to insert new row in dispatch table
			if ($insertDisapatchSuccess === false) {
				echo "Error: " . $sql . "<br>" . $conn -> error;
			}
		}
		...
		// if able to insert new row in incident and update patrolcar table and insert new row in dispatch table
		if ($insertIncidentSuccess === true && $updateSuccess === true && $insertDisapatchSuccess === true) {
	...
	<div class="container" style="width: 80%;">
		...
		<section style="margin-top: 20px;">
			<form action="dispatch.php" method="post">
				...
				<!-- Row to display Contact Name -->
				<div class="form-group row">
					<label for="contactNo" class="col-sm-4 col-form-label"> Contact Name</label>
					<div class="col-sm-8">
						<?php echo $contactNo; ?>
					...
					<div class="col-sm-8">
						<table class="table table-striped">
							<tbody>
								...
								<?php 
									// use for loop to populate the table row with patrolcar details retrieved from db
